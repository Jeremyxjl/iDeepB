...
def iDeepB_integrated_gradients(model, input_tensor, baseline=None, steps=50):
    ...
    integrated_grads_global = (input_tensor - baseline) * integrated_grads
    
    return integrated_grads_global, integrated_grads

'''
# seq_index = 1 # 2 有motif
# GradientXInput Saliency IntegratedGradients GradientBasedMethod
attribution_matrix = iDeepB_integrated_gradients(model = model.to(args.device), input_tensor = subseqOH[seq_index].float().to(args.device)).cpu().detach().numpy()
...
def iDeepB_integrated_gradients_fast_interpret(model, input_tensor, baseline=None, steps=50):
    if baseline is None:
        baseline = torch.zeros_like(input_tensor)
...
def attributionFromSeq(interpMethod, device, input, baseline, model, topN = 10):
    ...
    if(interpMethod == "Top_IntegratedGradientsSum"):
        ...
        attributions = iDeepB_integrated_gradients_fast_interpret(model = model.to(device), input_tensor = input.float().to(device).squeeze(axis =0 ), baseline = baseline.squeeze(axis =0 ))
        contrib_scores_integration = attributions[0].unsqueeze(axis =0 ).cpu().detach().numpy()
...
def interpret1HeadFromSeq(seqTranscribed, window_size, model, method, codeModel = "OneHot", device = None):  #mode "Fragment" midBase
    ...
    if remainder:
        for subseqOHTurn in subseqOH[:-1].split(1,  0): # batch
            ...
            treat_predict = attributionFromSeq(interpMethod= method, device=device, input = input, baseline=baseline, model=model, topN = 10)
            temp.extend(treat_predict)

        treat_predict_end = attributionFromSeq(interpMethod= method, device=device, input = subseqOH[-1:].to(device), baseline=baseline, model=model, topN = 10)
        ...
        for subseqOHTurn in subseqOH.split(1000,  0): #预测转录本，数据太大，无法预
            treat_predict = attributionFromSeq(interpMethod= method, device=device, input = subseqOHTurn.to(device), baseline=baseline, model=model, topN = 10)
