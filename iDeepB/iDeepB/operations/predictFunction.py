...
# for two head
def predict2HeadW(seqTranscribed, window_size, signalList, model, mode, codeModel = "OneHot", device = None):  #mode "Fragment" midBase
    ...
    with torch.no_grad():
        ...
        if mode == "Fragment":
            if remainder:
                for subseqOHTurn in subseqOH[:-1].split(128,  0): # batch
                ...
                outputs_end = model(subseqOH[-1:]) #torch.Size([n, 101])
                w_end = outputs_end[2]
                ...
                for subseqOHTurn in subseqOH.split(128,  0): #预测转录本，数据太大，无法预
                    outputs = model(subseqOHTurn) #torch.Size([n, 101])
                    w = outputs[2]
                    w = w.unsqueeze(axis=1)
...
# for one head
def predict1Head(seqTranscribed, window_size, signalList, model, mode, codeModel = "OneHot", device = None, batch = 10):  #mode "Fragment" midBase
    ...
    with torch.no_grad():
        ...
        if mode == "Fragment":
            if remainder:
                for subseqOHTurn in subseqOH[:-1].split(batch,  0): # batch  1000
                ...
                # for remainder length
                treat_predict_end = model(subseqOH[-1:])
                treat_predict_end = treat_predict_end.detach().cpu().numpy().tolist()
            ...
            for subseqOHTurn in subseqOH.split(batch,  0): #预测转录本，数据太大，无法预 [0:-0] 非[1:-1]
                treat_predict = model(subseqOHTurn) #torch.Size([n, 101])
                treat_predict = treat_predict[:, window_size//2]
                treat_predict = treat_predict.detach().cpu().numpy().tolist()
...
##################################
# for two head
def predict2HeadWFromSeq(seqTranscribed, window_size, model, mode, codeModel = "OneHot", device = None):  #mode "Fragment" midBase
...
# for one head
def predict1HeadFromSeq(seqTranscribed, window_size, model, mode, codeModel = "OneHot", device = None):  #mode "Fragment" midBase
    ...
    with torch.no_grad():
        ...
        if mode == "Fragment":
            if remainder:
                ...
                # for remainder length
                treat_predict_end = model(subseqOH[-1:]) #torch.Size([n, 101])
                treat_predict_end = treat_predict_end.detach().cpu().numpy().tolist()
            ...
            temp = treat_predict_end[:, (window_size//2 + 1):].reshape(-1).tolist() + temp
